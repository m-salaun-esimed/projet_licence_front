...
class Favori {
    ...
    async init() {
        ...
        for (const favorite of response) {
            this.responseMovieInfo = await this.moviesModel.getMovieByIdMovieApi(favorite.idmovieapi)
                    ...
                    <div class="row">
                        <div class="col-6">
                          <button type="button" class="btn rounded-circle mt-2" style="background-color: #22303C" title="Supprimer aux favoris"
                            onclick="rouletteAleatoire.removeFavorite(${favorite.idmovieapi})" id="fav">
                            <img src="../images/heart-fill%20(1).svg" width="20" height="20" alt="Favori">
                        </button>

                        </div>
                        <div class="col-6">
                          <button type="button" class="btn rounded-circle mt-2"  style="background-color: #22303C; color: white"
                          onclick="favori.showModalMovie(${favorite.idmovieapi})">
                            Info
                          </button>
                        </div>
                     </div>               
        ...
        `;
            ...
            if (count === 5 || response.length - 1 === response.indexOf(favorite)) {
